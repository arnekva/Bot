 ...
 * @param command Funksjon som skal kjøres. Tar inn message, content og args
 * @param hideFromListing  Sett til true for å gjemme funksjonen fra '!mz help' listen.
 * @param isAdmin Sett til true for å kun la admins kjøre.
 * @param deprecated  Hvis du bytter commandnavn, kan du bruke denne på den gamle for å gi beskjed om at den gamle er utfaset (teksten som sendes inn her er navnet på nye command)
 * @param isSuperAdmin Sett til true for å kun la superadmins kjøre.
 */
...
export class Commands {
    ...
    constructor(client: Client, messageHelper: MessageHelper) {
        ...
        this.statusCommands = new StatusCommands(this.client, this.messageHelper)
    ...
    getAllCommands() {
        return [
            ...
            ...this.statusCommands.getAllCommands(),
    ...
    helperCommands = (rawMessage: Message, messageContent: string, args: string[]) => {
        ...
        if (this.getCommandCatgeories().includes(args[0])) {
            ...
            this.getCommandCatgeories().forEach((category) => {
