...
export class Music extends AbstractCommands {
    ...
    async findCommand(message: Message, content: string, args: string[], params?: musicParams) {
        if (!args[0]) {
        ...
        if (args[0] != 'user') {
            ...
            let username = this.getLastFMUsernameByDiscordUsername(params.shouldEditRawMessageInstead ?? message.author.username, message)
            ...
            let limit = args[2] ?? '10'
            if (!username) {
                if (!params.silent) message.reply("Du har ikke registrert brukernavnet ditt. Bruk '!mz musikk user <discordnavn> <last.fm navn>")
                return
            }
            ...
            const dataRet = await this.findLastFmData(message, data, params.shouldEditRawMessageInstead, params.notWeeklyOrRecent, params.isSilent)
            return dataRet
        } else {
            if (args[1] && args[2]) {
