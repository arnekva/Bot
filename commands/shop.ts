...
export namespace ShopItems {
    export const items: shopItem[] = [
        { name: '0.5 Pils', description: 'Standard', price: 500 },
...
export class ShopClass {
    ...
    static async openShop(interaction: CommandInteraction, client: Client) {
        ...
        let shopDescription = 'Velkommen til Mazarini shop! \n \n Handleliste:'
        ...
        const menu = new MessageSelectMenu().setCustomId('MenyValg').setPlaceholder('Ingen gjenstander valgt!').setMinValues(1).addOptions(options)
        ...
        if (interaction.commandId === '879251024475467807') {
            let inventoryDescription = 'Inventory:'
            let debuffDescription = 'Debuffs:'
        ...
        if (interaction.commandId === '879333334784823316') {
            ...
            if (!inventoryItems || Object.values(inventoryItems).length === 0) {
                await interaction.reply('Du har ingenting i inventory?')
                return
            }
        ...
        if (interaction.isButton()) {
            ...
            } else interaction.reply({ content: 'Du må starte din egen interaksjon', ephemeral: true })
        }
    }
